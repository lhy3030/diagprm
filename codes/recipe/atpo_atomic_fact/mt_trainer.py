...
from recipe.atpo_atomic_fact.algo.med_algos import compute_grpo_outcome_advantage
from recipe.atpo_atomic_fact.mt_metric_utils import (
...
def pad_to_world_size(batch: DataProto, world_size: int) -> DataProto:
    current_len = len(batch)
    remainder = current_len % world_size
    
    if remainder == 0:
        return batch 

    pad_num = world_size - remainder
    device = batch.batch.device if batch.batch is not None else "cpu"
    pad_indices = torch.randint(low=0, high=current_len, size=(pad_num,), device=device)
    padding_part = batch.select_idxs(pad_indices)
    
    padded_batch = DataProto.concat([batch, padding_part])
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            input_texts = [self.tokenizer.decode(ids, skip_special_tokens=False) for ids in prompt_ids]
        ...
        if val_data_dir:
            self._dump_generations(
                inputs=[re.sub(f'^({re.escape(self.tokenizer.pad_token)})+', '', x) for x in sample_inputs],
                outputs=[re.sub(f'({re.escape(self.tokenizer.pad_token)})+$', '', x) for x in sample_outputs],
                gts=sample_gts,
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            from recipe.atpo_atomic_fact.agent_loop import AgentLoopManager

            self.async_rollout_mode = True
            if self.use_critic and self.config.extra_params.use_critic_in_loop:
                self.async_rollout_manager = AgentLoopManager(
                    config=self.config,
                    worker_group=self.actor_rollout_wg,
                    critic_work_group=self.critic_wg
                )
            else:
                self.async_rollout_manager = AgentLoopManager(
                    config=self.config,
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            ...
                            gen_batch_output, repeat_time_list = self.async_rollout_manager.generate_sequences(gen_batch)
                        timing_raw.update(gen_batch_output.meta_info["timing"])
                        metrics.update(gen_batch_output.meta_info["statistics_metric"])
                    ...
                    batch = pad_to_world_size(batch, self.actor_rollout_wg.world_size)
                    ...
                    if self.use_critic:
                        with marked_timer("values", timing_raw, color="cyan"):
                            if 'value_response_mask' in batch.batch.keys():
                                ...
                                old_response_mask = batch.batch["response_mask"]
                                batch.batch["responses"] = batch.batch["value_response_mask"]
                                batch.batch["response_mask"] = batch.batch["value_response_mask"]

                                values = self.critic_wg.compute_values(batch)

                                batch.batch["responses"] = old_responses
                                batch.batch["response_mask"] = old_response_mask
                            else:
                                values = self.critic_wg.compute_values(batch)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "advantages" in batch.batch.keys() and "returns" in batch.batch.keys():
                            print("Found pre-computed advantages in batch. Using them directly.")
                            
                            batch.batch["token_level_rewards"] = batch.batch["token_level_scores"]

                        else:
                            print("Advantages not found in batch, computing now using standard method...")
                            ...
                            # Compute advantages and returns using the configured estimator (GAE, GRPO, etc.)
                ...
                metrics.update(compute_accuracy_metrics(batch=batch))
