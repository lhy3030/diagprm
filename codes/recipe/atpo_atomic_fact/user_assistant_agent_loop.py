...
from recipe.atpo_atomic_fact.chat_model import (
    ChatModel,
    MaxTokenExceededError,
    convert_to_agent_output,
)
from recipe.atpo_atomic_fact.agent_loop import AgentLoopBase, AgentLoopOutput
from recipe.atpo_atomic_fact.api_request_async import request_vllm_async
from recipe.atpo_atomic_fact.tree_search_manager import TreeSearchManager, TreeNode
from recipe.atpo_atomic_fact.mt_reward_fn import calculate_single_turn_reward
...
def extract_answer(text):
    """Return the ATPO action body for both think and no-think Qwen output."""
    text = str(text or "")
    match = re.search(r"<think>[\s\S]*?</think>([\s\S]*)$", text, re.IGNORECASE)
    action = match.group(1) if match else text
    action = action.replace("<|im_end|>", "").strip()
    if action.startswith("Question:") or action.startswith("Final Answer:"):
        return action
    return None
...
def _set_current_turn_on_latest_user(
    messages: list[BaseMessage], current_turn: int, max_turns: int
) -> None:
    for message in reversed(messages):
        ...
        return
    raise ValueError("ATPO AtomicFact rollout requires a user message.")
...
async def call_critic_model(trajectory: list[BaseMessage], is_terminal: bool, config: dict) -> float:
    ...
    try:
        ...
        return 0.0
    # Treat the complete assistant generation prefix as the critic response.
    # Qwen3 no-think templates append `<think>\n\n</think>\n\n` after the
    # historical five-token `<|im_end|>\n<|im_start|>assistant\n` suffix, so
    # hard-coding five token ids is not portable across template revisions.
    # Derive the assistant prefix by comparing the same non-empty conversation
    # with and without a generation prompt. Transformers does not accept an
    # empty conversation in apply_chat_template().
    openai_trajectory = [
    ...
    context_ids = actor_model._apply_chat_template(
        openai_trajectory, add_generation_prompt=False
    )
    context_with_prefix_ids = actor_model._apply_chat_template(
        openai_trajectory, add_generation_prompt=True
    )
    if context_with_prefix_ids[:len(context_ids)] != context_ids:
        raise ValueError("Chat template generation prompt is not a suffix")
    response_ids = context_with_prefix_ids[len(context_ids):]
    if not response_ids or prompt_ids[-len(response_ids):] != response_ids:
        raise ValueError(
            f"Unexpected critic generation prefix: actual={prompt_ids[-len(response_ids):]}, "
            f"expected={response_ids}"
        )
    prompt_ids = prompt_ids[:-len(response_ids)]
    critic_input = {
        'request_id': request_id_base,
        'prompt_ids': prompt_ids,
        'response_ids': response_ids
    }
    value_result = await critic_worker.predict.remote(critic_input)
...
async def call_api(state: dict, config: dict):
    ...
    if isinstance(atomic_facts, np.ndarray):
        atomic_facts = atomic_facts.tolist()
    if isinstance(atomic_facts, (list, tuple)):
        atomic_facts = '\n'.join(atomic_facts)
    ...
    api_response = _strip_thinking(api_response)
    verifier_response = _strip_thinking(verifier_response) if verifier_response else verifier_response
...
class TreeSearchRunner:
    ...
    async def _expand_leaves_with_assistant(self) -> Dict[TreeNode, List[BaseMessage]]:
        ...
        for leaf in self.active_leaves:
            current_turn = 1 + sum(message.type == "ai" for message in leaf.messages)
            _set_current_turn_on_latest_user(
                leaf.messages,
                current_turn=current_turn,
                max_turns=self.config["configurable"]["max_assistant_turns"],
            )
...
class UserAssistantAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> list[AgentLoopOutput]:
        ...
        initial_messages = kwargs.get("raw_prompt")
        if initial_messages is None:
            initial_messages = kwargs.get("prompt")
        if initial_messages is None:
            initial_messages = extra_info.get("prompt")
        if initial_messages is None:
            raise ValueError("ATPO AtomicFact requires the dataset prompt field.")

        atomic_facts = extra_info.get("atomic_facts", kwargs.get("atomic_facts"))
        if atomic_facts is None:
            raise ValueError("ATPO AtomicFact requires extra_info.atomic_facts.")
        ground_truth = (kwargs.get("reward_model") or {}).get("ground_truth") or {}
        critic_worker = kwargs.get("critic_worker")
        ...
        model = ChatModel(
            ...
            enable_thinking=str(os.environ.get("ATPO_DOCTOR_THINKING", "0")).lower()
            not in {"0", "false", "no"},
        )
        ...
        config = {
            "configurable": {
                ...
                "apikey": kwargs.get("api_key") or os.environ.get("PATIENT_API_KEY", ""),
                "atomic_facts": atomic_facts,
                "ground_truth": ground_truth
            }
        }
        
        initial_messages = convert_to_messages(list(initial_messages))
